...
data Expr = Expr :+: Expr | Expr :-: Expr | Expr :*: Expr | Expr :/: Expr | Neg Expr | Const TCons | IdVar Id Tipo | Chamada Id [Expr] | Lit String deriving Show
...
chamadaFuncao = do
  i <- identificador
  args <- parenteses (list expr)
  let checkType = IdVar i <$> getType i
  return (Chamada <$> checkType <*> pure args)

idVarExpr = do
  i <- identificador
  let checkType = IdVar i <$> getType i
  return (IdVar <$> checkType)
...
getType :: Id -> Either String Tipo
getType "int" = Right TInt
getType "double" = Right TDouble
getType "string" = Right TString
getType "float" = Right TFloat
getType id = Left ("Variable '" ++ id ++ "' has an undefined type.")
